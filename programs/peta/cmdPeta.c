...
void readPetaFromFile(Matrix *container, char *filename) {
    ...
    // BACA JUMLAH BARIS DAN KOLOM PADA AWAL FILE, KONVERSI KE INTEGER (row,
    // col)
    row = 0, col = 0;
    for (i = 0; i < currentWord.Length; i++) {
        row += (currentWord.TabWord[i] - 48) *
               power(10, currentWord.Length - i - 1);
    }
    ADVWORD();
    for (i = 0; i < currentWord.Length; i++) {
        col += (currentWord.TabWord[i] - 48) *
               power(10, currentWord.Length - i - 1);
    }
    IgnoreBlanks();
...
boolean isPositionAvailable(POINT pos, Matrix peta) {
    // ALGORITMA
    if (ROW(pos) > 0 && ROW(pos) < COL_EFF(peta) - 1 && COL(pos) > 0 &&
        COL(pos) < ROW_EFF(peta) - 1) {
        if (ELMT(peta, ROW(pos), COL(pos)) == ' ' ||
            ELMT(peta, ROW(pos), COL(pos)) == 'S') {
            return true;
        }
        return false;
    }
...
boolean isAdjacent(POINT pos, Matrix peta, char object) {
    // ALGORITMA
    return (ELMT(peta, ROW(pos) + 1, COL(pos)) == object ^
...
boolean move(POINT *pos, Matrix *peta, char direction) {
    ...
    if (isPositionAvailable(temp, *peta)) {
        // Karakter 'S' pada posisi sebelumnya dihapus dan karakter 'S'
        // diletakkan di posisi baru
...
void moveTo(POINT startPos, POINT endPos, Matrix *peta) {
    ...
    // ALGORITMA
    if (isPositionAvailable(endPos, *peta)) {
        // Karakter 'S' pada posisi sebelumnya dihapus dan karakter 'S'
        // diletakkan di posisi baru
        ELMT(*peta, ROW(startPos), COL(startPos)) = ' ';
        ELMT(*peta, ROW(endPos), COL(endPos)) = 'S';
    }
}
