...
void printListMakanan(ListStatik l) {
    /* Proses : Menuliskan isi List dengan traversal, List ditulis di antara
       ...
       dan enter */
    /* I.S. l boleh kosong */
    /* F.S. Jika l tidak kosong tercetak semua informasi makanan yang ada di
     * dalam l */
    /* KAMUS LOKAL */
    IdxType i;
...
boolean isListEqualStat(ListStatik l1, ListStatik l2) {
    /* Mengirimkan true jika l1 sama dengan l2 yaitu jika ukuran l1 = l2 dan
    ...
    if (listLengthStat(l1) == listLengthStat(l2)) {
        ...
        while (i < listLengthStat(l1) && equal) {
            if (!stringsAreEqual((ID(MAKANAN(ELMTSTAT(l1, i)))),
                                 (ID(MAKANAN(ELMTSTAT(l2, i)))))) {
                equal = false;
            } else {
                i++;
            }
...
int indexOfMakanan(ListStatik l, String searchID) {
    ...
    /* Jika ada, menghasilkan indeks i terkecil, dengan ID(ELMTSTAT(l,i)) =
     * searchId */
...
void insertAtStat(ListStatik *l, ElTypeStat val, IdxType idx) {
    ...
    /* I.S. List l tidak kosong dan tidak penuh, idx merupakan index yang valid
     * di l */
...
ListStatik listMakananCommand(String com, ListStatik l) {
    /* Mengembalikan list statik yang berisi makanan dengan cara mendapat sesuai
     * com dari list l */
    /* KAMUS LOKAL */
    ListStatik result;
    int i;
